...
try {
    ...
    // Analytics may be blocked in local/file contexts; auth should still work.
}
...
function wireUi() {
    ...
    window.addEventListener("error", (e) => {
        setStatus(e?.message ? `Error: ${e.message}` : "An unexpected error occurred.");
    });

    window.addEventListener("unhandledrejection", (e) => {
        const msg = e?.reason?.message || String(e?.reason || "");
        setStatus(msg ? `Error: ${msg}` : "An unexpected error occurred.");
    });
    ...
    if (signInForm) {
        signInForm.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                setStatus(`Sign-in failed: ${normalizeAuthError(err)}`);
            }
        });
    }
...
try {
    ...
    const msg = e?.message || "Error initializing admin auth UI.";
    setStatus(`Error: ${msg}`);
}
